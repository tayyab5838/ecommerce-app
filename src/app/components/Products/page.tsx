...
const Products = () => {
  ...
  const handleSlideClick = (itemId: string) => {
    if (!isDraggingRef.current) {
      ...
      window.location.href = `/components/OrderPage/${itemId}`;
    }
  };
      ...
      <div>
        <Slider className="mt-6" {...settings}>
          {allData.map((item: any) => (
            <div
              key={item._id}
              className="scale-90 cursor-pointer transition-transform hover:scale-100 ease-in duration-300 focus:outline-none"
